-- Admin Panel v1: allow active admins to prepare team-managed reviews.

create or replace function public.admin_prepare_team_review(
  p_review_id uuid,
  p_rating integer,
  p_review_text text,
...
begin
  ...
  if v_review.source <> 'team' then
    raise exception 'only_team_reviews_can_be_prepared' using errcode = '42501';
  end if;

  if v_review.status not in ('awaiting_team', 'prepared') then
...
revoke execute on function public.admin_prepare_team_review(
  uuid,
  integer,
  text,
  text
...
grant execute on function public.admin_prepare_team_review(
  uuid,
  integer,
  text,
  text
